...
impl GameState {
    // ==================== Tick Flow: 串联一整帧的核心结算顺序 ====================
    // 这一段负责组织单帧推进流程：处理尸块、规划移动、判定碰撞、
    // 结算头撞头、推进存活蛇、处理死亡，并在最后补齐物品和递增 tick。
    ...
    // ==================== State Transitions: 把结算结果写回游戏状态 ====================
    // 这一段负责把已经判定好的结果落地，包括推进蛇身、消费落点效果、
    // 拆解尸体，以及推进尸块腐化与敌蛇重生等状态变化。
    ...
    // ==================== Collision & Board Rules: 判定移动、占位与死亡条件 ====================
    // 这一段回答“下一步会发生什么”：包括坐标推进、边界判断、尾巴占位规则、
    // 撞墙/撞身体/撞炸弹判定，以及头撞头胜负结算。
    ...
    // ==================== Item & Corpse Lifecycle: 管理物品、尸块与随机生成 ====================
    // 这一段负责棋盘资源的完整生命周期：补齐和消费食物/炸弹、拆分尸块、
    // 腐化生成遗留食物、统计空位，以及随机挑选新的可用生成位置。
